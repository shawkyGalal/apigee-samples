...
ENVIRONMENT_NAME=$ENV_NAME  #"iam-protected"
RUNTIME_HOST_ALIAS=$ENV_GROUP_DNS  #"apis.moj.gov.sa" 
NETWORK=$VPN  #"default"
SUBNET=$VPN_SUBNET  #"default"
CRED_FILE=$KEY_FILE # "./service-account-keys/postman@moj-apigee.iam.gserviceaccount.com.json"
PROJECT=$(cat "$CRED_FILE" | jq --raw-output '.project_id')
...
LOAD_BLANCER_VIP_NAME=$ENVIRONMENT_NAME"-vip"
SSL_CERTIFICATE_NAME=$ENVIRONMENT_NAME"-ssl-cert"
NETWORK_ENDPOINT_GROUP=$ENVIRONMENT_NAME"-neg"
BACKEND_SERVICE_NAME=$ENVIRONMENT_NAME"-backend-service"
URL_MAP_NAME=$ENVIRONMENT_NAME-"urlmap"
TARGET_HTTPS_PROXY_NAME=$ENVIRONMENT_NAME"-https-proxy"
FORWARDING_RULE_NAME=$ENVIRONMENT_NAME"-https-lb-rule"


# Create and attach a sample Apigee environment
echo -n "Creating environment..."
OPERATION=$(apigeecli environments create -o "$PROJECT" -e "$ENVIRONMENT_NAME" -d PROXY  -t "$TOKEN" | jq --raw-output '.name' | awk -F/ '{print $4}')
